...
import (
	...
	"extension-scaffold/tools/pkg/contracts/solvra"
...
func DeployInstructionSender(s *support.Support) (common.Address, *solvra.SolvraInstructionSender, error) {
	...
	address, tx, contract, err := solvra.DeploySolvraInstructionSender(
...
func SetExtensionId(s *support.Support, instructionSenderAddress common.Address) error {
	sender, err := solvra.NewSolvraInstructionSender(instructionSenderAddress, s.ChainClient)
	if err != nil {
	...
	if err != nil {
		...
		if reason == "" {
			parsed, _ := solvra.SolvraInstructionSenderMetaData.GetAbi()
			if parsed != nil {
